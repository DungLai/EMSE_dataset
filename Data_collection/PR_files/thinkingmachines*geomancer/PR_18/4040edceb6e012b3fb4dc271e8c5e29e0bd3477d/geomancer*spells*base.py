...
class Spell(abc.ABC):
    ...
    def query(self, source, target):
        """Sets the statement used to extract features
    ...
    def cast(self, df, client, bq_options=BQConfig, **kwargs):
        ...
        query = select(
            [
                ...
                func.row_number()
                .over(
                    partition_by=query.c["__index_level_0__"],
                    # TODO: order_by should be a parameter
                    order_by=query.c[self.feature_name].desc(),
                )
                .label("row_number"),
            ]
        ).select_from(query)
        query = select(
            [literal_column("* EXCEPT (__index_level_0__, row_number)")],
