    ...
    def query(self, source, target, core):
        ...
        lois = select(
            [
                source.c.osm_id,
                source.c.WKT,
                core.GeoFromText(source.c.WKT.label('geom'))
            ],
        ...
        centroids = select(
            [
                target.c[self.column].alter(name='target_wkt'),
                core.GeoFromText(target.c[self.column].label('geom'))
            ]
        )
        .cte('centroids')
        ...
        query = (
            ...
            .select_from(sum_length)
            .join(sum_length.c.fid == buff.c.fid)
        )
