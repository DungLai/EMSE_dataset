...
class HampelFilter(_SeriesToSeriesTransformer):
    ...
    def _compare(self, value, cv_median, cv_sigma):
        ...
        int/float or np.nan
            ...
            else np.nan (or True if return_bool==True)
        """
        if np.abs(value - cv_median) > self.n_sigma * cv_sigma:
            return np.nan
        else:
            return value
