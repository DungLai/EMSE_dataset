...
class ForecastingPipeline(_Pipeline):
	...
	`fit(y, X, fh)`  - chnages state to `fitted`. Then writes to self: Sets self._is_fitted flag to True. Writes self._y and self._X with y and X, respectively. Sets self.cutoff and self._cutoff to last index seen in y. Sets fitted model attributes ending in â_â. Stores fh to self.fh if fh is passed.

	`predict(fh, X)` - Forecast time series at future horizon. Requires state to be âfittedâ.
	Access in self:
		Fitted model attributes ending in â_â. self.cutoff, self._is_fitted
	Writes to self:
