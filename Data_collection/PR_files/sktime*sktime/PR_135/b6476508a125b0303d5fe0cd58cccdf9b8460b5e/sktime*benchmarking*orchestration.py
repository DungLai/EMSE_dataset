...
class Orchestrator:
    ...
    def fit(self, save_fitted_strategies=True, overwrite_fitted_strategies=False):
    ...
    def predict(self, predict_on_train=False):
    ...
    def fit_predict(self, predict_on_runtime=True, predict_on_train=False, save_strategies=True):
        """
        ...
        for task, data, dts_loaded, strategy, cv_fold, train, test in self._iter():
            ...
            if predict_on_train:
                y_pred = np.array(strategy.predict(dts_loaded.iloc[train]), dtype=np.intp)
