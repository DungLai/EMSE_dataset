...
def _timedeltas_to_int(values, freq=None):
...
def _date_offsets_to_int(values):
    if not isinstance(values, Iterable):
        values = [values]
...
def _subtract_time(a, b):
    ...
    if isinstance(b, pd.Period):
        diff = _date_offsets_to_int(diff)
        ...
        diff = _timedeltas_to_int(diff, freq=freq)
