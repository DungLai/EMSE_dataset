...
class DynamicFactor(_StatsModelsAdapter):
    """Dynamic Factor Foracster .
      ...
      error_cov_type : {'scalar','diagonal','unstructured'} ,default = 'diagonal'
        The structure of the covariance matrix of the observation error term, where âunstructuredâ puts no restrictions on the matrix, âdiagonalâ requires it to be any diagonal matrix (uncorrelated errors), and âscalarâ requires it to be a scalar times the identity matrix.
      ...
      cov_type : {'opg','oim','approx','robust','robust_approx','none'} , default = 'opg'
        ...
        âoimâ for the observed information matrix estimator, calculated using the method of Harvey (1989)

        âapproxâ for the observed information matrix estimator, calculated using a numerical approximation of the Hessian matrix.

        ârobustâ for an approximate (quasi-maximum likelihood) covariance matrix that may be valid even in the presence of some misspecifications. Intermediate calculations use the âoimâ method.

        ârobust_approxâ is the same as ârobustâ except that the intermediate calculations use the âapproxâ method.

        ânoneâ for no covariance matrix calculation
      cov_kwds :dict or None , default = None
        âapprox_complex_stepâ : bool, optional - If True, numerical approximations are computed using complex-step methods. If False, numerical approximations are computed using finite difference methods. Default is True.

        âapprox_centeredâ : bool, optional - If True, numerical approximations computed using finite difference methods use a centered approximation. Default is False.
      method : str , 'lbfgs'
      ...
      full_output : bool , default = 1
        Set to True to have all available output in the Results objectâs mle_retvals attribute. The output is dependent on the solver.  
      disp : bool ,   defualt = 5
      ...
      optim_score : {'harvey','approx'} , default = None 
        The method by which the score vector is calculated. âharveyâ uses the method from Harvey (1989), âapproxâ uses either finite difference or complex step differentiation depending upon the value of optim_complex_step, and None uses the built-in gradient approximation of the optimizer.
      ...
      optim_hessian : {'opg','oim','approx'} , default = None
        âopgâ uses outer product of gradients, âoimâ uses the information matrix formula from Harvey (1989), and âapproxâ uses numerical approximation.
    ...
    def __init__(self,k_factors,factor_order,error_cov_type,error_order,error_var,enforce_stationarity,start_params,transformed,includes_fixed,cov_type,cov_kwds,maxiter,full_output,disp,callback,return_params,optim_score,optim_complex_step,optim_hessian,low_memory):
    ...
    def _fit(self, y, X=None, fh=None):
        self._forecaster = _DynamicFactor(y,k_factors=self.k_factors,factor_order=self.factor_order,exog=self.exog,error_order=self.error_order,error_var=self.error_var,error_cov_type = self.error_cov_type,enforce_stationarity=self.enforce_stationarity)
