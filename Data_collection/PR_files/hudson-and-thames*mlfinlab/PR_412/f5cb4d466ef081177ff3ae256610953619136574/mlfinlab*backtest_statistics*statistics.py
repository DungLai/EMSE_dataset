...
def all_bets_concentration(returns: pd.Series, frequency: str = 'M') -> tuple:
    ...
    * low positive_concentration â no right fat-tail of returns (desirable)
    * low negative_concentration â no left fat-tail of returns (desirable)
    * low time_concentration â bets are not concentrated in time, or are evenly concentrated (desirable)
    * positive_concentration == 0 â returns are uniform
    * positive_concentration == 1 â only one non-zero return exists
...
def information_ratio(returns: pd.Series, benchmark: float = 0, entries_per_year: int = 252) -> float:
    ...
    The excess return is measured as the portfolioâs return in excess of the benchmarkâs
...
def probabilistic_sharpe_ratio(observed_sr: float, benchmark_sr: float, number_of_returns: int,
    ...
    PSR estimates the probability that SR Ìis greater than a hypothetical SR.
...
def deflated_sharpe_ratio(observed_sr: float, sr_estimates: list, number_of_returns: int,
    ...
    adjusted to reflect the multiplicity of trials. DSR is estimated as PSR[SRâ], where
    the benchmark Sharpe ratio, SRâ, is no longer user-defined, but calculated from
    SR estimate trails.
...
def minimum_track_record_length(observed_sr: float, benchmark_sr: float,
    ...
    that  the  observed Sharpe ratio Ìis above the designated Sharpe ratio threshold.
