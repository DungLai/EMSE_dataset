...
class TestVariableNames(unittest.TestCase):
    ...
    def _test_non_ascii_variable_name(self):
        ...
        model_onnx = to_onnx(
            ...
            initial_types=[("å¹´é½¢", FloatTensorType([None, X.shape[1]]))],
            target_opset=TARGET_OPSET)
        sess = InferenceSession(model_onnx.SerializeToString())
    ...
    def test_non_ascii_variable_name_pipeline(self):
        ...
        data.rename(columns={"age": "å¹´é½¢"}, inplace=True)
        X = data.drop('survived', axis=1)
        # y = data['survived']
        cols = ['embarked', 'sex', 'pclass', 'å¹´é½¢', 'fare']
        X = X[cols]
        for cat in ['embarked', 'sex', 'pclass']:
            X[cat].fillna('missing', inplace=True)
        numeric_features = ['å¹´é½¢', 'fare']
        ...
        initial_type = [('pclass', Int64TensorType(shape=[None, 1])),
                        ...
                        ('å¹´é½¢', FloatTensorType(shape=[None, 1])),
