...
#                                 Ludwig configs.
# âââ __init__.py             <-- Contains fully assembled Ludwig schema (`get_schema()`), `validate_config()` for YAML
#                                 validation, and all "top-level" schema functions.
# âââ utils.py                <-- An extensive set of marshmallow-related fields, methods, and schemas that are used
#                                 elsewhere in Ludwig.
# âââ trainer.py              <-- Contains `TrainerConfig()` and `get_trainer_jsonschema`
# âââ optimizers.py           <-- Contains every optimizer config (e.g. `SGDOptimizerConfig`, `AdamOptimizerConfig`,
#                                 etc.) and related marshmallow fields/methods.
# âââ combiners/
#     âââ __init__.py         <-- Imports for each combiner config file (making imports elsewhere more convenient).
#     âââ utils.py            <-- Location of `combiner_registry`, `get_combiner_jsonschema()`, `get_combiner_conds()`
#     âââ base.py             <-- Location of `BaseCombinerConfig`
#     âââ comparator.py       <-- Location of `ComparatorCombinerConfig`
#     ... <file for each combiner> ...
#     âââ  transformer.py     <-- Location of `TransformerCombinerConfig`
#
