...
res_post = [
    ...
    (re.compile(r'\\\\'''), r'\''),
    (re.compile(r'^\'([^\']+)$'), r'\1'),
    (re.compile(r'([\<\>\{\}\[\]\(\)\-\+\=:;,\./\?\!\$%&Â£#@\'â¹ ])\1+'), r'\1'),
    (re.compile(
        r'[^qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890\<\>\{\}\[\]\(\)\-\+\=:;,\./\?\!\$%&Â£#@\'â¹ ]'),
     r' '),
    (re.compile(r'\s{2,}'), r' ')
]
...
def clean_html(html_text):
    ...
    #     'qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM1234567890\<\>\{\}\[\]\(\)\-\+\=:;,\./\?\!\$%&â¬Â£#@'â¹\' ')
    # if result:
    #     print(result)
