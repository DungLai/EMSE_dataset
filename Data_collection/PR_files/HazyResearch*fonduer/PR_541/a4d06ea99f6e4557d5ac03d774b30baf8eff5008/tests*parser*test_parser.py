...
def test_parse_hocr():
    ...
    # "ã«ã£ã½ã" is tokenized into three: "ã«", "ã£", "ã½ã" in hOCR,
    # but it is tokenized as an one token by spaCy.
    assert sent.words[1] == "ã«ã£ã½ã"
    assert sent.left[1] == 150  # this left comes from "ã«" in hOCR
    assert sent.right[1] == 249  # this right comes from "ã½ã" in hOCR

    sent = doc.sentences[2]
    assert len(sent.words) == len(sent.left)
    # "ãã§ã¼ã³åº\næ¬ã»æ¯åº" is tokenized into three: "ãã§ã¼ã³åº", "æ¬ã»æ¯åº" in hOCR,
    # but it is tokenized as "ãã§ã¼ã³", "åºæ¬", "ã»", "æ¯åº" by spaCy.
    assert sent.words[1] == "åºæ¬"
    assert sent.left[1] == 145  # comes from left min of "ãã§ã¼ã³åº\næ¬ã»æ¯åº" in hOCR
    assert sent.right[1] == 245  # comes from right min of "ãã§ã¼ã³åº\næ¬ã»æ¯åº" in hOCR
